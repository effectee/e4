...
@Controller
@RequestMapping("e4/gv/enum/{key}")
public class EnumFieldRestController {

    @Autowired
    private IEnumVarService enumVarService;


    @RequestMapping(value="field/{code}",method=RequestMethod.GET)
    @ResponseBody
    public EnumVar get(@PathVariable("key") String key, @PathVariable("code") String code){
